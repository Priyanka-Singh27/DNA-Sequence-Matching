...
char* suffix_tree_search_to_json(STSearchResult* result, const char* pattern) {
    if (!result || result->count == 0) return NULL;
    
    int capacity = 128 + (result->count * 16);
    char* json = malloc(capacity);
    if (!json) return NULL;
    
    int offset = snprintf(json, capacity, "{\"count\": %d, \"matches\": [", result->count);
    
    for (int i = 0; i < result->count; i++) {
        offset += snprintf(json + offset, capacity - offset, "%d%s", 
                           result->positions[i], 
                           (i == result->count - 1) ? "" : ", ");
    }
    
    snprintf(json + offset, capacity - offset, "]}");
    return json;
}
